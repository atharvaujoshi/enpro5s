...
export const sendUploadNotification = async (zoneName, photoType, workId) => {
  ...
  try {
    ...
    console.log(`${photoType} upload notification sent for ${zoneName}`);
  } catch (error) {
    console.error(`Failed to send ${photoType} upload notification for ${zoneName}:`, error);
  }
};

export const sendCompletionNotification = async (zoneName, workRecord) => {
